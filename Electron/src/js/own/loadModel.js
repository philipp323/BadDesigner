...
async function showObjects(animationObj) {
  console.log(objects);
    ...
    console.log("Next Object: " + nextObject.name);
...
function generateObjects(id, objectParam){
  var generatedObjects = [];
  console.log(objectParam);
  if(objectParam != undefined){
    for (let index of objectParam.indexesOfGeometrysToGenerate) {
      var o = objectParam.children[0].children[index];
      ...
      objectsAll[id].detailViews.find(dV => dV.id == objectParam.generatedDetailViewId).array[0].objectsToDisplay.push(object);
    }
  }
...
function loadModel(object_name, detailLoad, detailViewId, objectToHideNumber, isTransparent, number, normalLoad, textureJSON, ANIMATION_MODE, HEADER, TEXT, isSUBPOINT, TIME, clockMinutes, isDetailBefore,
  ...
  async function onLoadComplete(){
    ...
    if(otherObjectsToGenerate){
      if(newObject.name.charAt(7) == "1"){
        generateObjects(0, newObject);
      } else if(newObject.name.charAt(7) == "2"){
        generateObjects(1, newObject);
      } else if(newObject.name.charAt(7) == "3"){
        generateObjects(2, newObject);
      } else if(newObject.name.charAt(7) == "4"){
        generateObjects(3, newObject);
      }
    }
    if(generateArrows){
    ...
    if(!newObject.normalLoad){
      ...
      if(newObject.name.charAt(7) == "1"){
        grundriss_id = 0;
        objectsAll[0].objects.push(newObject);
        if(objectsAll[0].numberOfModels == objectsAll[0].objects.length){
          new Promise( () => {
            loadTextures(0);
          } );
        }
        ...
        grundriss_id = 1;
        objectsAll[1].objects.push(newObject);
        if(objectsAll[1].numberOfModels == objectsAll[1].objects.length){
          new Promise( () => {
            loadTextures(1);
          } );
        }
        ...
        grundriss_id = 2;
        objectsAll[2].objects.push(newObject);
        if(objectsAll[2].numberOfModels == objectsAll[2].objects.length){
          new Promise( () => {
            loadTextures(2);
          } );
        }
        ...
        grundriss_id = 3;
        objectsAll[3].objects.push(newObject);
        if(objectsAll[3].numberOfModels == objectsAll[3].objects.length){
          new Promise( () => {
            loadTextures(3);
          } );
          objectsAll[3].objects.forEach(x => x.visible = true);
          renderer.clear();
      ...
      if(generateArrows){
        objectsAll[grundriss_id].detailViews.find(dV => dV.id == newObject.DETAIL_VIEW_ID).array.forEach(item => {
          if(item.arrowData.length != 0){
            item.arrows = generateRedArrows(item.arrowData);
          }
        });
      ...
      if(otherObjectsToDisplay){
        newObject.otherObjectsToDisplay = objectsAll[grundriss_id].detailViews.find(dV => dV.id == newObject.DETAIL_VIEW_ID).array[0].objectsToDisplay;
      }
      // newObject.visible = false;
  ...
  manager.onLoad = async function() {
    ...
    if(newObject.normalLoad){
      ...
      if(newObject.name.includes("4")){
        newObject.visible = true;
        floor = newObject;
      }
      if(newObject.name.includes("1")){
        objectsAll[0].floor = newObject;
      } else if(newObject.name.includes("2")) {
        objectsAll[1].floor = newObject;
      } else if(newObject.name.includes("3")) {
        objectsAll[2].floor = newObject;
      } else if(newObject.name.includes("4")) {
        objectsAll[3].floor = newObject;
      }
    }
    if(!newObject.normalLoad){
      objects.push(newObject);
